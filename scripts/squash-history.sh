...
squash_history_squash() {
  ...
  # Build a single parentless commit from the current tree and point the
  # branch at it in place. Unlike the checkout --orphan / branch -D dance,
  # this never deletes the branch, so its upstream tracking config survives.
  git -C "${REPO_ROOT}" add -A
  local tree new_commit
  tree="$(git -C "${REPO_ROOT}" write-tree)"
  new_commit="$(git -C "${REPO_ROOT}" commit-tree "${tree}" -m "${message}")"
  git -C "${REPO_ROOT}" reset --hard "${new_commit}"

  squash_history_info "Squashed ${branch} to one commit: $(git -C "${REPO_ROOT}" rev-parse --short HEAD)"
  squash_history_info "NOTE: git status will show ${branch} as diverged from the remote"
  squash_history_info "(e.g. 'ahead 1, behind ${commit_count}'). That is expected; do NOT pull or"
  squash_history_info "reset to the remote — that restores the old history and the pruned files."
...
squash_history_push() {
  ...
  squash_history_info "Will run: git push --force-with-lease ${remote} ${branch}"
